...
int GetAddresWithRetrys(char mensaje[],int reintentos, char mensajeError[],char *pLetrasIngresadas,int longitud){
	int rtn = -1;
	char auxiliarCharAndInt[1000];
	int auxiliarReintentos;
	int returnGetArrayCharsAndInt;
	...
	if(mensaje!= NULL && mensajeError != NULL && pLetrasIngresadas != NULL && auxiliarReintentos > 0){
		printf("%s\n",mensaje);
		do{
			returnGetArrayCharsAndInt = GetAddres(auxiliarCharAndInt,sizeof(auxiliarCharAndInt));
			if(returnGetArrayCharsAndInt ==1){
				rtn =1;
				auxiliarReintentos =0;
				strncpy(pLetrasIngresadas,auxiliarCharAndInt,longitud);
...
int GetAddres(char *pLetrasTomadas,int longitud){
	int rtn;
	char textoAuxiliar[1000];
	if(MyGets(textoAuxiliar,sizeof(textoAuxiliar))==1 && ((EsAddres(textoAuxiliar)==1))){
		strncpy(pLetrasTomadas,textoAuxiliar,longitud);
		rtn = 1;
	}
	else{
